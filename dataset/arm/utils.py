...
def discretize_euler(euler_angles, resolution):
    # print ("minimum euler angle ", euler, np.min(euler),  np.max(euler))
    ...
    angle_range = (-360, 360)
    num_bins = resolution  # 10-degree bins

    # Discretize the Euler angles
    bins = np.linspace(angle_range[0], angle_range[1], num_bins+1)
    discretized_angles = np.digitize(euler_angles_deg, bins, right=True)

    # Convert discretized angles back to radians if needed
    # discretized_angles_rad = np.radians(bins[discretized_angles])
